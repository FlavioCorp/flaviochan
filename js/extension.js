...
Parser.parseThread/jsON = function(data) {
  var thread;
...
Parser.parseCatalog/jsON = function(data) {
  var catalog;
...
Parser.buildPost = function(thread, board, pid) {
  ...
  for (i = 0; j = thread[i]; ++i) {
    ...
    el = Parser.buildHTMLFrom/jsON(j, board, false, true).lastElementChild;
...
Parser.buildHTMLFrom/jsON = function(data, board, standalone, fromQuote) {
  var
...
var Search = {
  ...
  buildResults: function(data) {
    ...
    for (j = 0; thread = threads[j]; ++j) {
      ...
      cnt.appendChild(Parser.buildHTMLFrom/jsON(op, thread.board, true));
      
      for (k = 1; reply = thread.posts[k]; ++k) {
        cnt.appendChild(Parser.buildHTMLFrom/jsON(reply, thread.board));
      }
...
Depager.renderNext = function() {
  ...
  for (j = 0; op = threads[j]; ++j) {
    ...
    cnt.appendChild(Parser.buildHTMLFrom/jsON(op, Main.board, true));
    ...
    if (op.replies) {
      ...
      for (k = 0; reply = last_replies[k]; ++k) {
        cnt.appendChild(Parser.buildHTMLFrom/jsON(reply, Main.board));
      }
    }
...
Depager.onLoad = function() {
  ...
  if (this.status == 200) {
    ...
    catalog = Parser.parseCatalog/jsON(this.responseText);
...
QuoteInline.inlineRemote = function(link, board, tid, pid) {
  ...
  onload = function() {
    ...
    if (this.status === 200 || this.status === 304 || this.status === 0) {
      thread = Parser.parseThread/jsON(this.responseText);
...
ThreadWatcher.fetchCatalog = function(board, catalogs, meta) {
  ...
  xhr.onload = function() {
    meta.count--;
    catalogs[board] = Parser.parseCatalog/jsON(this.responseText);
    if (!meta.count) {
...
ThreadWatcher.fetch = function(key, img) {
  ...
  xhr.onload = function() {
    ...
    if (this.status == 200) {
      posts = Parser.parseThread/jsON(this.responseText);
  ...
  $.get('//a.4cdn.org/' + Main.board + '/thread/' + tid + './json',
    {
      onload: function() {
        ...
        if (this.status == 200) {
          ...
          if (post) {
            post = Parser.buildHTMLFrom/jsON(post, Main.board);
      ...
      onload: function() {
        ...
        if (this.status == 200) {
          ...
          if (tail[0]) {
            ...
            for (i = 1; p = posts[i]; ++i) {
              if (p.no < tail) {
                n = Parser.buildHTMLFrom/jsON(p, Main.board);
...
ThreadUpdater.onload = function() {
  ...
  if (this.status == 200) {
    newposts = Parser.parseThread/jsON(this.responseText);
    ...
    if (count) {
      ...
      for (i = nodes.length - 1; i >= 0; i--) {
        frag.appendChild(Parser.buildHTMLFrom/jsON(nodes[i], Main.board));
      }
