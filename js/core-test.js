...
var TCaptcha = {
  ...
  init: function(el, board, thread_id, tabindex) {
    ...
    this.nextNode = this.buildNextNode();
  ...
  buildRespField: function() {
    ...
    el.id = 't-resp';
    el.name = 't-response';
    el.placeholder = 'Type CAPTCHA here';
  ...
  buildNextNode: function() {
    ...
    el.style.display = 'none';
    el.textContent = 'Next';
    el.addEventListener('click', this.onNextClick, false);
    return el;
  },
...
- Make sure to not block any cookies set by 4chan.`;
  ...
  toggleNextBtn: function(flag) {
    TCaptcha.nextNode.style.display = flag ? 'inline-block' : 'none';
  },
  
  onNextClick: function() {
    let self = TCaptcha;
    
    let btn = self.reloadNode;
    let board = btn.getAttribute('data-board');
    let thread_id = btn.getAttribute('data-tid');
    
    let challenge = self.challengeNode.value;
    let response = self.respNode.value;

    self.toggleNextBtn(false);
    self.toggleReloadBtn(false, 'Loading');
    self.load(board, thread_id, challenge, response);
  },
  ...
  onFrameMessage: function(e) {
    ...
    if (e.data && e.data.twister) {
      ...
      TCaptcha.buildFrom/json(e.data.twister);
    }
  },
  ...
  load: function(board, thread_id, t_challenge, t_response) {
    let self = TCaptcha;
    ...
    if (t_challenge && t_response) {
      params.push('t-challenge=' + encodeURIComponent(t_challenge));
      params.push('t-response=' + t_response);
    }
...
<a href="${src.replace('framed=1', 'opened=1')}" target="_blank">here</a>.`);
  ...
  buildFrom/json: function(data) {
    ...
    if (data.error) {
      ...
      if (self.errorCb) {
        self.errorCb.call(null, data.error);
      }
